...
import type { CourtDay, CourtCase, CourtDayStatus, LastAction, MatterType } from '../types';
...
export function deriveActiveCase(cd: CourtDay, view: ViewContext): ActiveCaseView | null {
  ...
  return {
    ...
    matterTypeLabel: c.matterType ? MATTER_TYPE_LABELS[c.matterType] ?? c.matterType : undefined,
    status: c.status,
...
export function deriveTimeBandGroups(cd: CourtDay, view: ViewContext): TimeBandGroup[] {
  ...
  return TIME_BANDS.map((band) => {
    const items = queue.filter((q) => {
      const m = q.estimatedMinutes ?? 5; // default 5 if unknown
...
/** Colour-coded duration: green ≤5, amber 6–20, red >20 */
