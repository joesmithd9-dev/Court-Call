...
async function loadCourtDayWithItems(courtDayId: string) {
  const [courtDay, items] = await prisma.$transaction([
    prisma.courtDay.findUniqueOrThrow({
      where: { id: courtDayId },
      include: { court: true },
    }),
...
export async function getPublicProjection(
  ...
  return mapPublicProjection(courtDay.court, courtDay, items);
}
...
export async function getRegistrarProjection(
  ...
  const { courtDay, items } = await loadCourtDayWithItems(courtDayId);
  return mapRegistrarProjection(courtDay.court, courtDay, items);
}
